...
<div class="flex min-h-screen items-center justify-center px-4">
	<div class="glass w-full max-w-md rounded-[var(--radius)] p-8">
		<div class="mb-8 text-center">
			<Logo size="lg" />
			<p class="mt-2 text-sm text-muted-foreground">Войдите в аккаунт</p>
		</div>
		...
		<form onsubmit={handleSubmit} class="space-y-4">
			...
			<div>
				...
				<Input id="password" type="password" bind:value={password} placeholder="••••••••" required class="rounded-xl border-[var(--glass-border)] bg-white/60" />
			</div>
		...
		<p class="mt-6 text-center text-sm text-muted-foreground">
			Нет аккаунта? <a href="/register" class="font-medium text-accent hover:underline">Создать</a>
		</p>
	</div>
</div>
